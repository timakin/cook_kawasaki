...
get_hikarie_meshi_response = (res) ->
  GURUNAVI_URL = GURUNAVI_ORIGIN_URL + "keyid=" + GURUNAVI_ACCESS_KEY + '&latitude=35.65898718965676&longitude=139.70277630100122&range=4&freeword=' + encodeURIComponent("ヒカリエ");
...
get_meshi_response = (res) ->
  GURUNAVI_URL = GURUNAVI_ORIGIN_URL + "keyid=" + GURUNAVI_ACCESS_KEY + '&latitude=35.65898718965676&longitude=139.70277630100122&range=4&freeword=' + encodeURIComponent(res.match[1]);
  gnavi.get_page_max(GURUNAVI_URL)
    .then (page_max) => gnavi.get_restaurants_info(page_max, GURUNAVI_URL)
    .then (promise_array) =>
      Promise.all(promise_array).then (results) =>
